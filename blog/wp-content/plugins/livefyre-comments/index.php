...
function livefyre_get_activity_id() {
    ...
    return $wpdb->get_var($wpdb->prepare(
        "SELECT lf_activity_id FROM $livefyre_activity_table ORDER BY lf_activity_id DESC LIMIT 1"));
}
...
function livefyre_check_lastid_umapped() {
    ...
    if ($cid!=null) {
        $lf_wp_id=$wpdb->get_var($wpdb->prepare(
            "SELECT wp_comment_id FROM $livefyre_activity_table ORDER BY lf_activity_id DESC LIMIT 1"));
        return $lf_wp_id != $cid;
    }
