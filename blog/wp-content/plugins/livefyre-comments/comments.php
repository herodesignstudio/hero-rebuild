<?php 
	global $livefyre_admin_url, $livefyre_bootstrap_url, $livefyre_http_url, $livefyre_plugin_version;
...
<?php } else { 
    if (livefyre_show_comments()) { 
        ...
        $url = $livefyre_bootstrap_url.'/api/v1.1/public/bootstrap/html/'.get_option( 'livefyre_blogname' ).'/'.base64_encode($post_id).'.html?allow_comments=' . comments_open();
        $request = new WP_Http;
        $result = $request->request( $url, array( 'method' => 'GET' ) );
        if ( is_array( $result ) && $result['response'] && $result['body'] ) {
            echo $result['body'];
        }
		?>
		</div>
